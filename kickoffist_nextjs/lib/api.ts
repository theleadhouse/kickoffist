...
// ─── VERIFIED FIXTURES — FIFA.com + ESPN + BBC Sport ─────────────────────────
// All UTC times verified. Results updated June 17 2026.
const FIXTURES: {
  ...
  {id:2,  h:"South Korea",  a:"Czechia",        utc:"2026-06-12T02:00:00Z", group:"A",stage:"Group Stage",venue:"Estadio Akron",                city:"Guadalajara",          res:[2,1],goals:[{team:"South Korea",player:"Son",minute:34},{team:"South Korea",player:"Hwang",minute:67},{team:"Czechia",player:"Schick",minute:80}]},
...
export function getStaticTodayMatches(): Match[] {
  return getStaticWCMatches().filter(m => m.istDate === todayIST());
}
export function getStaticTomorrowMatches(): Match[] {
  return getStaticWCMatches().filter(m => m.istDate === tomorrowIST());
}
...
// World Cup 2026 league ID on this API — we fetch by date
async function rapidApiFetch(endpoint: string): Promise<unknown> {
  const r = await fetch(`https://${RAPIDAPI_HOST}${endpoint}`, {
    headers: {
...
export async function getLiveMatches(): Promise<Match[]> {
  try {
    if (!RAPIDAPI_KEY) return getStaticWCMatches().filter(m => m.status === "LIVE");
    const d = await rapidApiFetch("/football-get-all-livescores") as {
      response?: { id:number; homeTeam:{name:string}; awayTeam:{name:string}; homeGoals:number; awayGoals:number; minute:string; league:{name:string}; events?:{type:string;player:string;time:string;team:string}[] }[]
    };
    const wc = (d.response || []).filter((m:any) =>
      m.league?.name?.toLowerCase().includes("world cup") ||
      m.league?.id === 1 || m.league?.id === 2000
    );
    ...
    return wc.map((m: any) => ({
      ...
      istTime: toISTTime(new Date().toISOString()),
      istDate: todayIST(),
      istDateLabel: "Live",
      ...
      group: "",
      minute: parseInt(m.minute) || undefined,
      goals: (m.events || [])
    ...
    return getStaticWCMatches().filter(m => m.status === "LIVE");
  }
}

// Auto-fetch today's results from RapidAPI to keep scores updated
export async function getTodayResultsFromAPI(): Promise<Partial<Record<string, {home:number;away:number;goals?:{team:string;player:string;minute:number}[]}>>> {
  try {
    if (!RAPIDAPI_KEY) return {};
    const today = new Date().toISOString().slice(0,10);
    const d = await rapidApiFetch(`/football-get-fixtures-by-date?date=${today}`) as {
      response?: {id:number;teams:{home:{name:string};away:{name:string}};goals:{home:number|null;away:number|null};fixture:{status:{short:string}};events?:any[]}[]
    };
    const results: Record<string, {home:number;away:number;goals?:{team:string;player:string;minute:number}[]}> = {};
    for (const m of (d.response || [])) {
      if (m.fixture?.status?.short === "FT" && m.goals.home !== null && m.goals.away !== null) {
