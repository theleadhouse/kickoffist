      ...
      <div className="card" style={{overflow:"hidden"}}>
        {GROUPS.map(({id,teams},gi) => (
          <div key={id} style={{borderTop:gi>0?"1px solid rgba(255,255,255,.05)":"none"}}>
            <div style={{display:"flex",justifyContent:"space-between",padding:"5px 12px",background:"rgba(255,153,51,.05)"}}>
              <span style={{fontFamily:"'Teko',sans-serif",fontSize:"12px",fontWeight:600,color:"#FF9933",letterSpacing:".1em"}}>GROUP {id}</span>
              <span style={{fontSize:"8px",color:"rgba(200,212,232,.25)"}}>MP PTS</span>
            </div>
            {teams.map((t,i) => (
              <div key={t.t} className="st-row">
                <span style={{fontSize:"9px",color:"rgba(200,212,232,.25)",width:"14px",textAlign:"center"}}>{i+1}</span>
                ...
                <span style={{flex:1,fontFamily:"'Teko',sans-serif",fontSize:"13px",fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",
                  color:i<2&&t.pts>0?"#fff":"rgba(200,212,232,.4)"
                }}>{t.t}</span>
                <span style={{fontSize:"10px",color:"rgba(200,212,232,.25)",width:"18px",textAlign:"center"}}>{t.p}</span>
                <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"15px",letterSpacing:".04em",width:"22px",textAlign:"center",
                  color:t.pts>0?"#FF9933":"rgba(200,212,232,.2)"
                }}>{t.pts}</span>
              </div>
            ))}
          </div>
        ...
        <div style={{padding:"6px 12px",borderTop:"1px solid rgba(255,255,255,.05)",display:"flex",alignItems:"center",gap:"5px"}}>
          ...
          <span style={{fontSize:"8px",color:"rgba(200,212,232,.2)"}}>Top 2 qualify · 4 teams per group</span>
        </div>
      </div>
    </div>
  );
