...
export default function PortalMatchCard({ match, showDate=false }: { match: Match; showDate?: boolean }) {
  ...
  const toggleExpanded = () => {
    if (isFinished || isLive) setExpanded(!expanded);
  };

  const rowCursor = (isFinished || isLive) ? "pointer" : "default";
  ...
  return (
    <div className={`mc ${isLive ? "mc-live" : ""}`}>
      ...
      {/* ── MAIN ROW ── */}
      <div
        style={{ display:"flex", alignItems:"stretch", minHeight:"78px", cursor:rowCursor }}
        onClick={toggleExpanded}
      >
        {/* Colour strip */}
        <div style={{
          width:"4px", flexShrink:0, borderRadius:"14px 0 0 14px",
          background: isLive ? "#ef4444" : isFinished ? "rgba(74,222,128,.5)" : "#FF9933"
        }}/>

        {/* Group / status meta */}
        <div style={{
          width:"70px", flexShrink:0, display:"flex", flexDirection:"column",
          alignItems:"center", justifyContent:"center", padding:"8px 4px", gap:"4px",
          borderRight:"1px solid rgba(255,255,255,.05)", background:"rgba(0,0,0,.15)"
        }}>
          {match.group && (
            <span className="badge-grp">{match.group.replace("Group ","Grp ")}</span>
          )}
          {isLive && (
          ...
          {isUpcoming && showDate && match.istDateLabel && (
            <span className="badge-up" style={{fontSize:"9px"}}>{match.istDateLabel.slice(0,6)}</span>
          )}
        </div>
        ...
        <div style={{
          ...
          borderLeft:"1px solid rgba(255,255,255,.05)", minWidth:"68px"
        }}>
          {isUpcoming && (
            <div style={{ textAlign:"right" }}>
              <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"20px", letterSpacing:".04em", color:"#FF9933", lineHeight:1 }}>
                {match.istTime}
              </div>
              <div style={{ fontSize:"9px", color:"rgba(255,255,255,.3)", marginTop:"2px" }}>IST</div>
              {countdown && (
                <div style={{ fontSize:"9px", color:"rgba(255,153,51,.4)", marginTop:"3px" }}>{countdown}</div>
              )}
            </div>
          )}
          {isLive && (
            <div style={{ textAlign:"right" }}>
              ...
              {match.minute && (
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"22px", letterSpacing:".04em", color:"#f87171", lineHeight:1 }}>
                  {match.minute}&apos;
                </div>
              )}
            </div>
          ...
          {isFinished && (
            <div style={{ textAlign:"right" }}>
              <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"16px", letterSpacing:".04em", color:"rgba(255,255,255,.35)" }}>
                {match.istTime}
              </div>
              <div style={{ fontSize:"9px", color:"rgba(255,255,255,.2)" }}>IST</div>
              <div style={{ fontSize:"11px", color:"rgba(255,255,255,.2)", marginTop:"4px" }}>
                {expanded ? "▲" : "▼"}
              </div>
            </div>
          )}
      ...
      {(isUpcoming || isFinished) && (
        <div style={{
          borderTop:"1px solid rgba(255,255,255,.05)", padding:"8px 12px",
          display:"flex", gap:"6px", background:"rgba(0,0,0,.1)"
        }}>
          {isUpcoming && (
            <button
              ...
              style={{
                ...
                background:"rgba(255,153,51,.08)", border:"1px solid rgba(255,153,51,.18)",
                borderRadius:"7px", padding:"7px", cursor:"pointer", transition:"all .15s"
              }}
            >
              <span style={{ fontSize:"13px" }}>⏰</span>
              <span style={{ fontFamily:"'Teko',sans-serif", fontSize:"12px", fontWeight:600, color:"#FF9933", letterSpacing:".06em" }}>
                SET ALARM
              </span>
            </button>
          )}
          {isFinished && (
            ...
            >
              <span style={{ fontSize:"13px" }}>⚽</span>
              <span style={{ fontFamily:"'Teko',sans-serif", fontSize:"12px", fontWeight:600, color:"rgba(200,212,232,.6)", letterSpacing:".06em" }}>
                GOALS
              </span>
            </button>
          )}
      ...
      {showAlarm && isUpcoming && (
        <div style={{
          borderTop:"1px solid rgba(255,153,51,.12)",
          background:"rgba(255,153,51,.05)", padding:"12px 14px"
        }}>
          <div style={{ fontFamily:"'Teko',sans-serif", fontSize:"12px", fontWeight:600, color:"#FF9933", letterSpacing:".1em", marginBottom:"10px" }}>
            ⏰ NEVER MISS THIS MATCH
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:"7px" }}>
            <a
              href={googleCalUrl(match)} target="_blank" rel="noopener noreferrer"
              style={{
            ...
            >
              ...
              <div>
                <div style={{ fontFamily:"'Teko',sans-serif", fontSize:"14px", fontWeight:600, color:"#fff", letterSpacing:".04em" }}>
            ...
            <a
              href={whatsappMsg(match)} target="_blank" rel="noopener noreferrer"
              style={{
                display:"flex", alignItems:"center", gap:"10px",
            ...
            >
              <span style={{ fontSize:"20px", flexShrink:0 }}>💬</span>
              <div>
                <div style={{ fontFamily:"'Teko',sans-serif", fontSize:"14px", fontWeight:600, color:"#25d366", letterSpacing:".04em" }}>
      ...
      {showShare && (
        <div style={{
          ...
          background:"rgba(0,0,0,.15)", padding:"12px 14px"
        }}>
          <div style={{ fontFamily:"'Teko',sans-serif", fontSize:"12px", fontWeight:600, color:"rgba(200,212,232,.6)", letterSpacing:".1em", marginBottom:"8px" }}>
            📤 SHARE THIS MATCH
          </div>
          <div style={{
            background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.07)",
            borderRadius:"8px", padding:"10px 12px", marginBottom:"8px",
            fontSize:"12px", color:"rgba(200,212,232,.7)", lineHeight:1.7, whiteSpace:"pre-wrap"
          }}>
            {shareText}
          </div>
          <div style={{ display:"flex", gap:"7px" }}>
            <button
              ...
              style={{
                flex:1, padding:"9px",
                background: copied ? "rgba(74,222,128,.1)" : "rgba(255,153,51,.1)",
                border: copied ? "1px solid rgba(74,222,128,.3)" : "1px solid rgba(255,153,51,.25)",
                borderRadius:"8px", cursor:"pointer",
                fontFamily:"'Teko',sans-serif", fontSize:"13px", fontWeight:600,
                color: copied ? "#4ade80" : "#FF9933", letterSpacing:".06em", transition:"all .15s"
              }}
            >
            ...
            <a
              ...
              style={{
                ...
                borderRadius:"8px", cursor:"pointer",
                fontFamily:"'Teko',sans-serif", fontSize:"13px", fontWeight:600,
                color:"#25d366", letterSpacing:".06em", textDecoration:"none",
      ...
      {expanded && (isFinished || isLive) && (
        <div style={{
          ...
          {(hGoals.length > 0 || aGoals.length > 0) ? (
            ...
            <div style={{ fontSize:"10px", color:"rgba(255,255,255,.25)", textAlign:"center" }}>
              Goalscorer data unavailable
            </div>
          )}
        </div>
      )}
