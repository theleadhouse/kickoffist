...
export default function ScheduleCalendar({ matches, played }: { matches: Match[]; played: number }) {
  ...
  const dates: string[] = [];
  matches
    .filter(m => m.homeTeam.name !== "TBD")
    .map(m => getISTDate(m.utcDate))
    .sort()
    .forEach(d => { if (!seen[d]) { seen[d] = true; dates.push(d); } });

  const filtered = matches
    .filter(m => getISTDate(m.utcDate) === sel && m.homeTeam.name !== "TBD")
