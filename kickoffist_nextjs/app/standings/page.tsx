  ...
  return (
    <div style={{ maxWidth:"1100px", margin:"0 auto" }}>
      <div style={{ marginBottom:"20px", paddingBottom:"14px", borderBottom:"2px solid rgba(26,107,26,.1)" }}>
        ...
        <p style={{ fontSize:"12px", color:"rgba(255,255,255,.3)" }}>Updated June 23 2026 · All 12 groups · 4 teams each · Verified: ESPN + Yahoo + FIFA.com</p>
      </div>

      {/* MY TEAMS — moved here from homepage */}
      <div style={{ marginBottom:"20px", background:"rgba(255,153,51,.04)", border:"1px solid rgba(255,153,51,.15)", borderRadius:"14px", padding:"14px 16px" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"8px" }}>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"18px", letterSpacing:"1px", color:"#fff" }}>
            ⭐ {myTeams.length > 0 ? "MY TEAMS" : "PICK YOUR TEAMS"}
          </div>
          <button onClick={()=>setShowPicker(!showPicker)} style={{ fontFamily:"'Teko',sans-serif", fontSize:"12px", fontWeight:600, color:"#FF9933", background:"none", border:"1px solid rgba(26,107,26,.2)", borderRadius:"6px", padding:"4px 10px", cursor:"pointer" }}>
        ...
        {myTeams.length > 0 && !showPicker && (
          <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
            {myTeams.map(t => {
              ...
              return (
                <div key={t} style={{ display:"flex", alignItems:"center", gap:"5px", background:"rgba(255,255,255,.05)", border:"1px solid rgba(255,255,255,.08)", borderRadius:"8px", padding:"5px 10px", boxShadow:"0 1px 4px rgba(26,107,26,.06)" }}>
        ...
        {(showPicker || myTeams.length === 0) && (
          <div>
            <p style={{ fontSize:"11px", color:"rgba(255,255,255,.3)", marginBottom:"10px" }}>Pick your teams — we highlight them in standings</p>
            <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
              {TEAMS_LIST.map(t => {
            ...
            <div key={id} className="card" style={{ overflow:"hidden" }}>
              <div style={{ padding:"10px 14px", background:"rgba(255,153,51,.06)", borderBottom:"2px solid rgba(255,153,51,.12)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <div style={{ display:"flex", gap:"8px", fontFamily:"'Teko',sans-serif", fontSize:"11px", color:"rgba(255,255,255,.3)", letterSpacing:".06em" }}>
                  ...
                  <span style={{ width:"22px", textAlign:"center", color:"#FF9933" }}>PTS</span>
                </div>
              </div>
              {sorted.map((t,i) => {
                const gd = t.gf-t.ga;
                ...
                return (
                  <div key={t.n} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"9px 14px", borderBottom:i<3?"1px solid rgba(26,107,26,.06)":"none", background: isMyTeam?"rgba(255,153,51,.05)":q?"rgba(26,107,26,.02)":"transparent" }}>
                    <span style={{ fontFamily:"'Teko',sans-serif", fontSize:"11px", color:"rgba(255,255,255,.3)", width:"14px", textAlign:"center" }}>{i+1}</span>
                    ...
                    <div style={{ display:"flex", gap:"8px", fontFamily:"'Teko',sans-serif", fontSize:"12px", color:"rgba(255,255,255,.3)" }}>
              ...
              <div style={{ padding:"5px 14px", display:"flex", alignItems:"center", gap:"5px", borderTop:"1px solid rgba(255,255,255,.05)" }}>
                <span style={{ width:"7px", height:"7px", borderRadius:"2px", background:"rgba(26,107,26,.3)" }}/>
                <span style={{ fontFamily:"'Teko',sans-serif", fontSize:"10px", color:"rgba(255,255,255,.3)" }}>Top 2 advance + 8 best 3rd place</span>
              </div>
            </div>
          );
        })}
      </div>
      <p style={{ fontSize:"10px", color:"rgba(255,255,255,.3)", textAlign:"center", marginTop:"20px" }}>Source: ESPN · Yahoo Sports · FIFA.com · June 23 2026 IST · 4 teams per group</p>
    </div>
  );
}
