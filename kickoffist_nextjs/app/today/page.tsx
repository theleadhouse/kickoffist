...
export default async function TodayPage() {
  ...
  const hasToday = live.length>0||todayUpcoming.length>0||todayLive.length>0;

  return (
    <div>
      {/* ── HERO — dark stadium pitch ── */}
      <div style={{
        ...
        {/* Pitch lines */}
        <div style={{position:"absolute",inset:0,backgroundImage:"repeating-linear-gradient(rgba(255,255,255,.03) 0,rgba(255,255,255,.03) 1px,transparent 1px,transparent 48px),repeating-linear-gradient(90deg,rgba(255,255,255,.03) 0,rgba(255,255,255,.03) 1px,transparent 1px,transparent 48px)"}}/>
        {/* Centre circle */}
        <div style={{position:"absolute",right:"-2%",top:"50%",transform:"translateY(-50%)",width:"50%",aspectRatio:"1",borderRadius:"50%",border:"1.5px solid rgba(255,255,255,.06)"}}/>
        <div style={{position:"absolute",right:"calc(25% - 1px)",top:0,bottom:0,width:"1px",background:"rgba(255,255,255,.04)"}}/>
        {/* Ball watermark */}
        <div style={{position:"absolute",right:"7%",top:"50%",transform:"translateY(-50%)",fontSize:"88px",opacity:.04,userSelect:"none",lineHeight:1}}>⚽</div>
        {/* Saffron floodlight */}
        <div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:"70%",height:"60%",background:"radial-gradient(ellipse at 50% 0%,rgba(255,153,51,.08) 0%,transparent 70%)"}}/>

        <div style={{position:"relative",zIndex:1,padding:"22px 20px 20px"}}>
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexWrap:"wrap",gap:"12px"}}>
            <div>
              <div style={{display:"inline-flex",alignItems:"center",gap:"7px",background:"rgba(255,153,51,.12)",border:"1px solid rgba(255,153,51,.25)",borderRadius:"6px",padding:"4px 12px",marginBottom:"12px"}}>
                <span>🏆</span>
              ...
              <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(28px,5vw,52px)",letterSpacing:"2px",color:"#fff",lineHeight:.9,marginBottom:"8px"}}>
              ...
              <p style={{fontSize:"12px",color:"rgba(255,255,255,.5)"}}>{dateStr} · Indian Standard Time 🇮🇳</p>
            </div>
            <div style={{display:"flex",gap:"8px"}}>
              {[{n:`${played}`,l:"Played"},{n:`${104-played}`,l:"Left"}].map((s,i)=>(
                <div key={i} style={{background:"rgba(0,0,0,.5)",border:"1px solid rgba(255,153,51,.15)",borderRadius:"10px",padding:"10px 14px",textAlign:"center"}}>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"26px",letterSpacing:"1px",color:"#FF9933",lineHeight:1}}>{s.n}</div>
                  <div style={{fontSize:"9px",color:"rgba(255,255,255,.4)",marginTop:"2px"}}>{s.l}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          {!hasToday&&!isMorning&&(
            <div className="card" style={{padding:"28px",textAlign:"center",marginBottom:"20px"}}>
              ...
              <div style={{fontFamily:"'Teko',sans-serif",fontSize:"18px",fontWeight:600,color:"rgba(255,255,255,.4)",marginBottom:"4px"}}>No matches today</div>
              <div style={{fontSize:"13px",color:"rgba(255,255,255,.25)"}}>Tomorrow&apos;s fixtures below</div>
            </div>
          )}
          ...
          <div className="card" style={{padding:"16px",marginBottom:"16px",background:"rgba(255,153,51,.04)",border:"1px solid rgba(255,153,51,.1)"}}>
            <div style={{fontFamily:"'Teko',sans-serif",fontSize:"12px",fontWeight:600,color:"#FF9933",letterSpacing:".12em",marginBottom:"10px"}}>📺 WATCH IN INDIA</div>
            <div style={{display:"flex",gap:"12px",alignItems:"center",flexWrap:"wrap"}}>
              <div style={{background:"rgba(0,0,0,.3)",border:"1px solid rgba(255,153,51,.15)",borderRadius:"12px",padding:"12px 18px",flex:1,minWidth:"160px"}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"22px",letterSpacing:"2px",color:"#FF9933",marginBottom:"3px"}}>ZEE5</div>
                <div style={{fontSize:"11px",color:"rgba(255,255,255,.6)",marginBottom:"2px"}}>Official exclusive broadcaster · India</div>
                <div style={{fontSize:"10px",color:"rgba(255,255,255,.3)"}}>zee5.com · Zee5 app</div>
              </div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
            {[
              ...
              {href:"/standings",icon:"📊",label:"Group Tables",   sub:"All 12 groups · Pick teams"},
              {href:"/news",     icon:"📰",label:"WC Stories",     sub:"Ronaldo 6 WCs · Messi record"},
            ].map(l=>(
